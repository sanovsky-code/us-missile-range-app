...
- **Windows + OneDrive + Next.js dev = unrecoverable jest-worker crashes**: dev shows `Jest worker encountered 2 child process exceptions, exceeding retry limit` on routes that worked moments earlier. Root cause: Next.js spawns jest-worker subprocess pools for parallel compilation (Turbopack AND webpack both, via SWC + PostCSS). On Windows those subprocesses fail to spawn with `0xc0000142` (DLL init failed) when OneDrive holds transient sync locks on files in `.next/` or `node_modules/`. After two failures the dev server wedges. **Fix is two-part and both halves are required**: (1) the `dev` script uses `next dev --webpack` to reduce jest-worker usage; (2) `scripts/setup-dev-cache.ps1` junctions BOTH `.next/` and `node_modules/` out of OneDrive to `%LOCALAPPDATA%\next-cache\<project>\`. The webpack switch alone wasn't enough (node_modules still hit lock contention); the .next-only junction wasn't enough (workers still read node_modules from OneDrive). Production builds (`next build`) write through the same junctions transparently; the dist build script reads `.next/standalone` and finds it. Run the setup script once after every fresh clone — see comment header in the script for invocation.
