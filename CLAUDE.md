...
- **Windows + OneDrive + Turbopack dev cache**: `.next/dev` MUST live outside OneDrive sync, otherwise Turbopack's jest-worker subprocesses crash mid-compile with `Jest worker encountered 2 child process exceptions, exceeding retry limit` (Windows error 0xc0000142 — DLL init failed while OneDrive holds a sync lock). Fix is a one-time per-checkout setup: stop the dev server, then run `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/setup-dev-cache.ps1`. It replaces `.next/` with a junction to `%LOCALAPPDATA%\next-cache\<project>\` and adds an inner `node_modules` junction back to the project (required so PostCSS / Tailwind can resolve modules from the AppData side). Production builds (`next build`) still write through the same junction; the dist build script reads `.next/standalone` transparently. The script refuses to run while node is alive — it can't kill its own npm host.
